...
export default function HoloEarth() {
    ...
    const render = () => {
      try {
        ...
        for (let lat = -60; lat <= 60; lat += 20) {
          ...
          ctx.beginPath();
          ctx.ellipse(centerX, y, Math.max(0, r), Math.max(0, r * 0.3), 0, 0, Math.PI * 2);
        ...
        for (let i = 0; i < longitudes; i++) {
          ...
          ctx.beginPath();
          ctx.ellipse(centerX + xOffset * 0.2, centerY, Math.max(0, radius * Math.cos(lonAngle)), radius, 0, 0, Math.PI * 2);
        ...
        console.warn('[HoloEarth Canvas Render Exception]', err);
      }
    };
  ...
  return (
    <div className="relative w-full h-full min-h-[250px] flex items-center justify-center pointer-events-none">
